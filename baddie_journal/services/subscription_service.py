...
from models.subscription import (
    ...
    SubscriptionPlanType, DEFAULT_PLANS
)
    ...
    def create_user_with_trial(
        ...
        try:
            ...
            if self.notification_service:
                self.notification_service.send_trial_starting_notification(user, subscription)
            
            logger.info(f"Created user {user.id} with trial subscription {subscription.id}")
    ...
    def create_trial_subscription(self, user: User) -> Subscription:
        ...
        if self.stripe_service and user.stripe_customer_id:
            try:
                ...
                logger.info(f"Created Stripe trial subscription {stripe_data['subscription_id']}")
    ...
    def upgrade_trial_to_paid(self, subscription: Subscription) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.stripe_service and subscription.stripe_subscription_id:
                ...
                new_period_end = datetime.now(UTC) + timedelta(days=30)  # 30 days from now
                subscription.upgrade_from_trial(new_period_end)
            
            # Update plan ID
            subscription.plan_id = basic_plan.id
            
            logger.info(f"Upgraded subscription {subscription.id} from trial to {basic_plan.id}")
                ...
                try:
                    ...
                    if result['success']:
                        ...
                        if self.notification_service:
                            ...
                            logger.info(f"Should send upgrade notification for subscription {subscription.id}")
                
                except SubscriptionError as e:
                    logger.error(f"Failed to upgrade expired trial {subscription.id}: {str(e)}")
