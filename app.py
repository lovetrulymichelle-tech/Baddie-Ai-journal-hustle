...
class JournalHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if path == '/api/add-entry':
            ...
            if content and mood and category:
                ...
                self.send_header('Location', '/journal')
                self.end_headers()
            else:
                self.send_error(400)
        else:
    ...
    def serve_journal(self):
        ...
        html = self.get_journal_html(entries)
        self.send_response(200)
        self.send_header('Content-type', 'text/html; charset=utf-8')
        self.end_headers()
        self.wfile.write(html.encode('utf-8'))
    ...
    def serve_insights(self):
        ...
        entries = get_all_entries()
        html = self.get_insights_html(entries)
        self.send_response(200)
        self.send_header('Content-type', 'text/html; charset=utf-8')
        self.end_headers()
        self.wfile.write(html.encode('utf-8'))
...
"""
    ...
    def get_home_html(self):
        ...
        """
        return self.get_base_html("Home", content)
    
    def get_journal_html(self, entries):
        ...
        """
        return self.get_base_html("Journal", content)
    
    def get_insights_html(self, entries):
        ...
        """
        return self.get_base_html("Insights", content)
    ...
    def get_about_html(self):
        ...
        """
        return self.get_base_html("About", content)
...
def run_server(port=8000):
    ...
    print(f"🌸 Baddie AI Journal running at http://localhost:{port}")
    print("✨ Pink luxury vibes activated! ✨")
    try:
