...
import sys
import os
sys.path.insert(0, os.path.join(os.path.dirname(os.path.abspath(__file__)), 'baddie_journal'))

from models.subscription import (
    User, Subscription, SubscriptionPlan, SubscriptionStatus, DEFAULT_PLANS
)
...
def demonstrate_subscription_plans():
    ...
    for plan in DEFAULT_PLANS:
        ...
        print(f"   Trial: {plan.trial_days} days" if plan.trial_days > 0 else "   No trial")
...
def demonstrate_trial_management(subscription_data):
    ...
    notification_service = NotificationService()
    notification_type = notification_service.should_send_trial_notification(subscription)
...
def demonstrate_subscription_upgrade(subscription):
    ...
    try:
        ...
        if result['success']:
            ...
            print(f"   Needs Upgrade: {'❌' if not access_info['needs_upgrade'] else '⚠️'}")
...
def demonstrate_notification_system():
    ...
    template = notification_service.get_notification_content(NotificationType.TRIAL_EXPIRING_3_DAYS)
